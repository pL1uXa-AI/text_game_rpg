...
# ── «Мир в ходу» (аудит 41, A5) ───────────────────────────────────────
# `player_turn()` говорит фону «модель занята, подожди» — это про ОЧЕРЕДЬ и он глобальный.
# Перемотке/загрузке сохранения нужно другое: «ЭТОТ мир прямо сейчас доигрывает ход», иначе
# откат таймлайна успевает состояться ДО того, как ход допишет события/setting — и мир
# «снова уезжает в будущее» (A5). Множество per-world, живёт здесь же, рядом с `_regen`:
# знают о нём и роутеры хода, и rewind.py (импорт routers/core создал бы цикл).
_turns: set[int] = set()


def is_turn_active(world_id: int) -> bool:
    """Идёт ли по миру ход игрока (перемотка/загрузка в это окно не допускаются)."""
    return int(world_id) in _turns


@contextlib.contextmanager
def turn_block(world_id: int, allow_regen: bool = False):
    """Занять мир на время хода игрока.

    yield False — и вызывающий обязан выйти, ничего не трогая, — если мир уже в ходу
    (задвоенный запрос) или над ним стоит барьер `_regen` (идёт ↻ либо откат таймлайна:
    дописывать поверх него = потерять/удвоить откатанное). `allow_regen=True` — для самого ↻,
    у которого барьер `_regen` занят НАМИ (снаружи): отказываем только параллельному ходу.
    """
    wid = int(world_id)
    if wid in _turns or (not allow_regen and wid in _regen):
        yield False
        return
    _turns.add(wid)
    try:
...
# ── Откат таймлайна: ⏪ перемотка и загрузка сохранения (аудит 41, A5) ────
# `rewind_block` пользуется ТЕМ ЖЕ барьером `_regen`, что и ↻: фоновые агенты (и Провидение)
# проверяют его через `_bg_may_write` / `is_regenerating`, поэтому импорт routers сюда не
# нужен, а «перемотал — и мир снова уехал в будущее» исчезает: пока идёт откат, ни один
# агент не допишет старое состояние и ни один ход игрока не стартует (он упрётся в 409).
@contextlib.contextmanager
def rewind_block(world_id: int):
    """Занять мир на время отката. yield False, если по миру идёт ход/↻ или откат уже
    выполняется (задвоенный клик) — вызывающий обязан выйти, ничего не трогая."""
    wid = int(world_id)
    if wid in _regen or wid in _turns:
        yield False
        return
    _regen.add(wid)
    try:
