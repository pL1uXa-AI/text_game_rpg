...
def build_system_prompt(world: dict, setting: dict, persona: str | None = None,
    ...
    lore_rules += (
        ...
        "обнови notes).\n"
        "36. РУЖЬЯ НА ГОРИЗОНТЕ (сессия 34): в состоянии может быть строка «🏹 На горизонте» — это то, "
        "что ты ввёл в мир (знакомство, вещь, факт, место) и о чём с тех пор не было речи. Хорошая "
        "привычка автора — вернуть такое в сюжет, когда это уместно: вернувшийся персонаж, "
        "сработавшая деталь, закрытый долг дают ощущение продуманного мира. Но это НЕ обязанность и "
        "НЕ очередь: ненужное ружьё имеет право так и не выстрелить, а список сам гаснет со временем. "
        "Не перечисляй ружья игроку списком — вплетай в повествование.\n"
    )
    if canon_note:
