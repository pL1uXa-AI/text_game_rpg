...
>
> Сюда же, в раздел «🔎 Аудит 41 (сессии 41–78)», пишется и НОВАЯ история: живые разделы
> `AGENT.md` держат только действующие правила, отчёт сессии сюда обязателен (инвариант C4,
> `tests/test_session62_docs_history.py`). Раздел «🧪 Тестовый набор» — поимённый состав
> pytest-набора и прирост по сессиям; актуальное ЧИСЛО тестов меряет не он, а
> `scripts/doc_figures.py` (архив вне его реестра намеренно).
...
## 🔎 Аудит 41 (сессии 41–78): ядро хода, гигиена, документы, «живой мир»

> Сводка написана по итогам работ, а не по чужому отчёту: каждый пункт закрыт регрессией,
> номер сессии в скобках — когда закрыт. Живые правила от этих сессий — в `AGENT.md`
> (жёсткие правила 5/6/7/9/14/15, инварианты 2/5/27/29/30/31); здесь — «симптом → причина →
> чем заплатили», чего в живом файле быть не должно (C4).

**Ядро хода и директивы (A1–A6, D1–D15):**

- **A1 (сессия 41): границы кубика.** `roll_expr` не знал потолков: `d0`/`0d6`/`d1`/«`2d6zz`»
  роняли ход на 500-ку либо «бросали ноль кубиков», а `99999999d20` раздувал событие и вешал
  цикл (пальцы в розетку: один `tool_call` от модели = DoS на весь ход). Стало: клампинг ввода
  (`DICE_COUNT_MAX=100`, `2 ≤ DICE_SIDES_MAX ≤ 1000`, `|mod| ≤ 1000`), невалидное → фолбэк на
  НАСТОЯЩИЙ d20 + `note` «подрезано» + `log.warning`, ход НЕ падает. Хвост того же пункта —
  D10 (сессия 72): `/roll` возвращал `state: None` (фронт не обновлял сайдбар) и не писал
  событие броска в память — RAG «не помнил» ключевых проверок.
- **A2 (сессия 42): экранирование модельных текстов во фронте.** Все тексты, приходящие от
  LLM и из пользовательских файлов (персона, лор, дампы импорта, имена карточек), вставлялись
  в `innerHTML` — рассказчик, которого попросили «<img src=x onerror=…>», исполнял код в
  браузере владельца. Закрыто `esc()` на всех точках вставки + двойная проверка:
  `check_frontend.xss_check` (по исходнику) и `xss_behavior_check` (НАСТОЯЩИЙ `renderSetting`
  на DOM-заглушке, `scripts/xss_probe.js`) — инвариант 20 в `AGENT.md`.
- **A3 (сессия 43): действие игрока читалось моделью дважды.** Событие действия записывалось
  в БД ДО сборки промпта и одновременно шло в блок `[ДЕЙСТВИЕ ИГРОКА]` → `_recent_block`
  возвращал его же в истории. Цена не только двойная трата `recent_token_budget`, но и
  провоцирование модели копировать собственную формулировку. Стало: `exclude_ids` в
  `_recent_block`; при `↻` отсекаются и прежнее действие, и `replaced_ids`.
  **ИНВАРИАНТ** (правило в `AGENT.md`): любой новый блок промпта из «своего» события хода обязан
  попасть в `exclude_ids` — иначе дефект вернётся молча.
- **A4 (сессия 44): судья логики судил по устаревшему состоянию.** `_maybe_logic_judge`
  честно перечитывал мир из БД в `s`, но LLM-проход звал со СТАРЫМ аргументом `setting`
  (снимок хода `agents_setting`). За `await sleep(1.5)` + ожидание bg-очереди мир успевал
  измениться → вердикт по фактам прошлого хода = ложные «искажения реальности» в чате.
  То же касалось мастера, боевого ИИ и видения — теперь ВСЕ фоновые агенты считают на
  перечитанном `s` (D6, сессия 68: параметр `setting` у них — остаток до-А4 эпохи, в расчёте
  не участвует; динамическое событие мира не получает и его — `_maybe_dynamic_event(world_id)`).
- **A5 (сессия 45): перемотка/загрузка сохранения шли поверх идущего хода.** Откат таймлайна
  не держал перемычку фона: идущий ход догонял откат и мир «сразу уезжал в будущее» —
  тихая порча прохождения вместо отказа. Стало: `bg.rewind_block` (та же `_regen`, что у `↻`)
  и `bg.turn_block` на ход игрока; занятый мир = честный 409.
- **A6 (сессия 46): правило 14 до конца.** Отказ фонового агента терялся в `except: pass`.
  Введено `narrator._agent_quiet()` — `log_once` с ключом (`агент`, `world`) и traceback-ом:
  молчание исключено, но и лог-шторм на каждом ходу тоже (агент живёт каждый ход, а при
  лежащей модели падает КАЖДЫЙ проход). Удалены два мёртвых замыкания `_parse()`
  (divine/logic_judge), `db.close()` пишет warning при залоченном на Windows `game.db`,
  отказ rate-limit виден в журнале. Попутно снято обещание дока о несуществующих хелперах
  `quiet()`/`log_debug_once()` в `logsetup` — docstring их больше не обещает (сторожится тестом).
- **A7–A13 (сессии 47–53):** `/journal` и категории дневника доезжают до SQL (`limit`/`cat`,
  а не «выбрать всё и отфильтровать в Python»; битая `meta` не роняет дневник); `GET /history?limit=0`
  больше не «весь лог» (потолок страницы + честный `truncated`); границы числовых настроек
  (кривое число из `.env`/админки больше не ломает все будущие ходы — реестр `NUM_RANGES`,
  кламп с видимой подрезкой в журнале); `/journal note` делегирует владельцу `journal.add_player_note`
  (раньше дублировал логику → разные ключи и дубли записей); событие `rewound` доходит до всех
  вкладок по шине (перезагрузка лога, а не дорисовка к удалённому прошлому); ОДИН критерий
  «жива ли модель» на весь проект (`up`/`needs_key`: 401 = ход работает, плашка врала);
  стартовое само-исцеление переехало с импорта модуля в `_lifespan` (импорт `backend.app` при
  занятой базе стоил 12.6 с и два таймаута вместо 0.5 с).

**Гигиена репозитория и доступность (B1–B5, сессии 54–58):** рабочий шаг `mypy -p backend` в CI
(позиционная форма падала «No parent module», а `continue-on-error: true` это скрывал — второй
раз за две сессии); `ratelimit.SCOPES` на всех 19 «дорогих» маршрутах + `require_local` для
админки + `REMOTE_FACTOR=4` для чужих адресов + отказ launcher'а выходить на чужой `GAME_BIND`
без `GAME_BIND_CONFIRM=1`; `max_length` на всех текстовых полях `schemas.py` (один POST с
мегабайтом раньше оседал в SQLite и уходил в промпт каждого хода) + `clip_persona` как страховка
на чтении; правда о мёртвом мусоре в рабочем дереве (сверяется ИНДЕКС, а не диск — правило 15);
снятие мёртвого/лишнего конфига (`RERANK_TOP_N`, `GAME_HOST`/`GAME_PORT`) через единый реестр
админки и `stale` в `GET /api/admin/settings`.

**Документы (C1–C4, сессии 59–62):** повторяющиеся числа в README/AGENT/ROADMAP сводит механизм,
а не сессия руками (`scripts/doc_figures.py`, `--sync`, падежи тоже); полнота таблицы API
сверяется с `app.openapi()`; латиница, приклеенная к кириллице, ловится сканером (`GLUE_RE` +
`GLUE_OK`, осознанная деградация: буквальный regex из задания давал 376 ложных попаданий по
одному `backend/`); живые документы перестали быть журналом сессий — отчёты уехали сюда.

**«Живой мир» (сессия 63)** — единственный не-дефектный пункт этого блока, сделанный по идее
владельца. Раньше сюжет был рельсами: `plots._theme_from_plot`/`theme_from_custom` приказывали
«идти строго в русле сюжета → вести к глобальной цели по канве», а у движка НЕ было `remove` для
локаций и квестов-арок — сожжённый город висел на карте вечно, а невозможную арку приходилось
«выполнять» (статистика вранья). Сделано (закон 3 не тронут: код хранит и проверяет, творит
мастер): правило **37** «ЖИВОЙ МИР vs КАНВА СЮЖЕТА» в `TAIL_RULES` (намеренно вне `_GATED_RULES`
— это не подсистема, нужно каждому миру), директивы `quest_remove` / `location_remove` (текущую
локацию игрока удалить нельзя — сперва `move`, это «код проверяет») и `world_evolve {what, why}`
+ `backend/plot_deviation.py` (чистое хранение) и `PlotDeviationHandler`. Стиль тем переформулирован:
канва — напряжение, а не маршрут (лор остаётся законом).
**Две цены, уплаченные за это:** (а) правило 37 стоит ~300 токенов, и каскад усечения
`build_messages` на узких окнах вырождался (лор/RAG/карточки/сводки/история срезаны, переполнение
остаётся) — добавлены два последних рычага: резка карточек **не из сцены** и удаление блока
примеров `<<ENGINE>>` (`drop_engine_examples` режет только строки с маркером, нумерацию правил
не сдвигает); тест `test_build_messages_never_overflows_window` до этого жил на грани 6192/6192;
(б) правка владельца по ходу сессии — отход от канвы **не показывается игроку**: и блок
`🧭 ЖИВОЙ МИР`, и запись — только заметки мастера (`PlotDeviationHandler` возвращает ПУСТОЙ
список сообщений): строка «мир отходит от канвы сюжета» в чате выдала бы существование заранее
написанного сценария и сломала бы иллюзию живого мира изнутри. Лимита частоты в правиле 37 нет
(«раз в 10–15 ходов» убрано как искуственная клетка).

**Хвосты D1–D15 (сессии 64–78):** честный `world_id=None` в `_context_guard` (в логе больше нет
несуществующего «world 0», к которому нельзя отнести строку); база записи Провидения — свежее
`fresh`, а не снимок до LLM-прохода (иначе правки фона зтирались «старым» состоянием);
`ratelimit` — обычные словари + амортизированная чистка несуществующих ключей (`defaultdict`
создавал запись на ЧТЕНИЕ, карта росла вечно); ОДНО условие «ответ оборван» вместо двух
алфавитов в одной строке (правка `_FINISH_CHARS` разъезжала бы их молча); исход броска по ФАКТУ
броска, а не по написанию/итому (nat-1 = грань из `rolls`, крит-провал даже с `mod +5`; форма
куба из той же регулярки, что распознавание: `d20` = `1d20` = `D20+3`); системки мира
(`🕐 Время` / `🌦 Погода` / `🚩 флаг` — только при РЕАЛЬНОЙ смене, иначе авто-часы давали дубль;
`station:`-флаги молчат — машинный ключ); `add_item`: ОДНА точка записи `value`/`weight` —
`_give_item` — и ОДНО сообщение на ход (раньше «выдай 1000 мечей» = 1000 строк в чате);
`db.update_world` — ValueError на неизвестном поле вместо тихого no-op; таймаут внутри
`db.transaction()` больше не оставляет соединение в открытом `BEGIN IMMEDIATE`; строка
длительности API-запроса несёт `world_id`; `metrics.record()` не делает дискового I/O на горячем
пути (журнал пишется пачками, формат не менялся); обе HTML-страницы отдаются с одинаковым
`Cache-Control: no-store` (отсюда снята жалоба правила 6 на «hard refresh»); финальный пункт —
обратная сверка векторов с БД (`memory.sweep_orphan_vectors()`, сессия 78; замер на боевой базе:
1103 вектора, 35 живых миров, 55 разных `world_id` — 23 удалённых мира держали 160 сиротских
записей, и RAG доставал сюжет удалённого мира).
...
*(из `AGENT.md`, «СОСТОЯНИЕ И ИСТОРИЯ → Контроль качества»: по сессиям и по файлам)*
  ...
  +10 — README не врёт про API: полнота таблицы по `app.openapi()`/C2 (сессия 60, в файле 10),
  +8 — латиница, приклеенная к кириллице/С3 (сессия 61),
  +8 — живые документы перестали быть журналом сессий/С4 (сессия 62),
  +23 — «живой мир»: сюжет как точка отсчёта, а не рельсы (сессия 63),
  +5/+5/+7/+17 — D1/D2/D3/D5: фейковый `world 0` в логе, Провидение на свежем состоянии,
  течь ключей rate-limit, одно условие «обрыва» вместо двух (сессии 64–67),
  +4/+32/+26/+20 — D6/D7/D8/D9: док и сигнатуры фоновых агентов, исход броска по факту,
  системки времени/погоды/флага, одна точка `value` и одно сообщение `add_item` (сессии 68–71),
  +8/+10/+14/+8/+17/+9 — D10–D15: `/roll` с состоянием и памятью, `update_world` не тихий
  no-op, таймаут не оставляет `BEGIN IMMEDIATE`, `world_id` в строке длительности,
  метрики без дискового I/O на ходу, одинаковый `Cache-Control` (сессии 72–77),
  +14 — сиротские векторы в Chroma (сессия 78). ИТОГО: 1051 тест — это и есть замер
  `doc_figures` на сегодня; «+N» считается по `pytest --collect-only -q` с РАЗДЕЛОМ на файл
  (параметризованные случаи дают больше, чем `def test_*` в файле, — поэтому сверяться надо
  с collect, а не с grep'ом).
  Состав ниже — дословный перенос из живого `AGENT.md` (С4, сессия 62); файлы сессий 61–78
  описаны в конце перечисления:
  ...
  `test_session61_latin_glue.py` (аудит 41, С3 — латиница в кириллице: исправленные цитаты,
  `GLUE_RE`+`GLUE_OK` в обе стороны, прогон сканера по всему проекту, `--probe`),
  `test_session62_docs_history.py` (аудит 41, С4 — границы переноса: бюджет веса живого AGENT.md
  и README, «в живом нет эссе сессий», «нет копий одного текста в двух файлах», «ссылки в архив
  ведут в существующие разделы», «архив вне реестра чисел `doc_figures`» + деградационные пробы
  на декоративность — ни одного изменения живых доков тест НЕ делает),
  `test_session63_living_world.py` (23 — живой мир: `quest_remove`/`location_remove`/`world_evolve`,
  кап 20 в `setting.plot_deviation`, «нельзя удалить текущую локацию», правило 37 вне `_GATED_RULES`
  и без лимита частоты, отход НЕ попадает в чат и НЕ виден игроку (`PlotDeviationHandler` →
  пустые системки), стиль темы называет канву точкой отсчёта, `test_build_messages_never_overflows_window`
  на узком окне с двумя новыми рычагами каскада),
  `test_session64_context_guard_world_id.py` … `test_session78_orphan_vectors.py` (аудит 41,
  D1–D15 + сиротские векторы: по одному файлу на пункт, состав и пробы — в докстрингах самих
  файлов; живого смысла в перенос их строк сюда нет — поведение описано блоком выше),
