...
и assert'ами сверяли подстроки — это «smoke по тексту»: регрессий в поведении cmd такой тест
...
которые ЧЕСТНО являются структурными инвариантами файла и должны жить в scripts/check_*:
  ...
  [3] сервер игры слушает ТОЛЬКО 127.0.0.1 (правило 5: наружу нельзя — нет авторизации),
      а любой чужой GAME_BIND требует явного GAME_BIND_CONFIRM=1 и НЕ рекламируется в
      выводимых строках (B2, аудит 41 — ветку поведением ловит bat-харнесс, на Linux,
      где cmd.exe нет, её видит этот чекер);
...
def check_main_bat(text: str) -> list[str]:
    ...
    # B2 (аудит 41): launcher не имеет права сам звать игрока «открыть наружу» и обязан
    # требовать явного подтверждения для не-loopback адреса. В CI (Linux) bat-харнесс
    # skip'ится, поэтому здесь — ЧЕСТНЫЙ структурный инвариант файла (см. докстринг модуля):
    # поведение ветки сторожит tests/test_start_bat_harness.py (на Windows).
    if "GAME_BIND_CONFIRM" not in text:
        errs.append("start_game.bat: нет гейта GAME_BIND_CONFIRM — чужой адрес поднимается без подтверждения (B2)")
    if re.search(r"^\s*(REM\s+)?echo\b.*0\.0\.0\.0", text, re.M | re.I):
        # объяснение отказа оперирует именем переменной; сам «открой наружу вот так» в
        # выводимых строках жить не должен (правило 5, B2)
        errs.append("start_game.bat: в echo снова есть приглашение запуститься на 0.0.0.0 "
                    "(вывод читает игрок, а не программист — REM-комментарий для этого) — B2")
